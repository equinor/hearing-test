...
function setNextNode(state, userResponse) {
  ...
  clonedState.node = userResponse.success ? clonedState.node.success : clonedState.node.failure;
  ...
  if (!(clonedState.node.success && clonedState.node.failure)) {
    ...
    const lastUserResponse = clonedState.userResponses[clonedState.userResponses.length - 1];
    ...
    clonedState.node = moreSubTests ? nextSubTest : { message: 'We are finished' };
...
export default handleActions(
  {
    ...
    [startTest]: state => {
      if (_.isEmpty(state.test)) {
        return {
          ...state,
          error: { message: 'Could not start the test. The test was not loaded correctly.' },
        };
      }
      return {
        ...state,
    ...
    [success]: (state, action) => setNextNode(state, { ...action.payload, success: true }),
